...
public class MainActivity extends AppCompatActivity
    ...
    private void populateCelebs() {
        ...
        celebList = dbhelp.queryForAllCelebRecords();}
            ...
            celebList.add(new Celebrity("Stephen Malkmus", "51", "Musician"));
            celebList.add(new Celebrity("Samuel L Jackson", "70", "Actor"));
            celebList.add(new Celebrity("Prince", "Deceased", "Musician"));
            celebList.add(new Celebrity("Trey Anastasio", "55", "Musician"));
        }
    }
    ...
    private void initializeRecyclerView(){
        ...
        RecyclerViewAdapter celebRecyclerViewAdapter = new RecyclerViewAdapter(celebList);
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_home) {
            //nothing happens if home is pressed...really only wanted a home button if this nav were across all activities
            //the reason why is because the only nav we have is on the home screen
        } else if (id == R.id.nav_add_celebrity) {

            Log.d(TAG, "onNavigationItemSelected: Inside second if");
            //if the ID of the nav drawer is clicked, send to the addceleb activity
            Intent add = new Intent(this, AddCelebrity.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == ADD_CELEB_REQUEST) {
            if(resultCode == RESULT_OK){
                Celebrity celeb = data.getExtras().getParcelable("result");
                celebList.add(celeb);
            }
        }
    }
